...
for(var i=max(0,curX-1);i<=min(obj_path.fieldWidth-1,curX+1);i++){
    for(var j=max(0,curY-1);j<=min(obj_path.fieldHeight-1,curY+1);j++){
        if(i==curX && j==curY)
            continue;
        ...
        var distFromCurrentToIJ=0;
         ...
         if(diagonal){
            canWalk=obj_path.walkable[i,j] &&
                obj_path.walkable[curX,j] && 
                obj_path.walkable[i,curY];
