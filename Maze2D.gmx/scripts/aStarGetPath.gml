...
startX=startRoomX div obj_path.blockSize;
startY=startRoomY div obj_path.blockSize;
endX=endRoomX div obj_path.blockSize;
endY=endRoomY div obj_path.blockSize;
...
if(found){
    ...
    while(curNode!=getKey(startX,startY)){
        path_add_point(path,getKeyX(curNode)*obj_path.blockSize,
            getKeyY(curNode)*obj_path.blockSize,100);
        curNode=ds_map_find_value(P,curNode);
    }
    path_add_point(path,startX*obj_path.blockSize,startY*obj_path.blockSize,100);
    path_reverse(path);
